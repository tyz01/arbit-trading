...
@Component
@AllArgsConstructor
public class ComparePricesFromDifferentExchanges {
    ...
    private static final int PERCENTAGE_VALUE = 0;

    private static final Set<String> allExlusiveExcgahgeName = new HashSet<>();
    ...
    private Map<String, BigDecimal> getDataFromCex(List<CexBean.PriceData> cexCurrencyList) {
        ...
        for (CexBean.PriceData cexData : cexCurrencyList) {
            allExlusiveExcgahgeName.add(cexData.getSymbol1());
    ...
    private Map<String, BigDecimal> getDataFromBinance(List<BinanceBean> binanceCurrencyList) {
        ...
        for (BinanceBean binancePriceData : binanceCurrencyList) {
            ...
            allExlusiveExcgahgeName.add(binancePriceData.getSymbol());
        }
    ...
    private Map<String, BigDecimal> getDataFromCurrency(List<CurrencyBean> currencyExchangeList) {
        ...
        for (CurrencyBean currencyExchangeData : currencyExchangeList) {
            ...
            allExlusiveExcgahgeName.add(currencyExchangeData.getSymbol());
        }
    ...
    private void comparePricesAndSendMessage(Map<String, BigDecimal> cexData, Map<String, BigDecimal> binanceData, Map<String, BigDecimal> currencyData) {
        ...
        allExlusiveExcgahgeName.stream()
                .filter(currency -> binanceData.containsKey(currency) || currencyData.containsKey(currency))
                .forEach(currency -> {
                    final BigDecimal cexPrice = cexData.get(currency);
                    final BigDecimal binancePrice = binanceData.getOrDefault(currency, BigDecimal.ZERO);
                    final BigDecimal currencyPrice = currencyData.getOrDefault(currency, BigDecimal.ZERO);
                    ...
                    if (isNewCurrency && (
                        ...
                        formatPriceDifferenceMessage(currency, diffPercentCexBinance, diffPercentCexCurrency, diffPercentBinanceCurrency, cexPrice, binancePrice, currencyPrice);
                        telegramSender.sendMessage(formatPriceDifferenceMessage(currency, diffPercentCexBinance, diffPercentCexCurrency, diffPercentBinanceCurrency, cexPrice, binancePrice, currencyPrice));
                    }
                });
    }
    ...
    private BigDecimal calculatePriceDifferencePercent(BigDecimal price1, BigDecimal price2) {
        if (price1 != null & price2 != null) {
            if (!price1.equals(new BigDecimal(0)) & !price2.equals(new BigDecimal(0))) {
